...
class IndigoAdaptor():
    ...
    def to_json(self, device):
        ...
        # periods as key delimiters don't travel well
        for key in newjson.keys():
            key: str
            newkey = key.replace(".", "_")
            if key != newkey:
                newjson[newkey] = newjson[key]
                del newjson[key]
    ...
    def diff_to_json(self, device):
        ...
        for kk, vv in newjson.iteritems():
            if kk not in localcache or localcache[kk] != vv and not isinstance(vv, indigo.Dict) and not isinstance(vv, dict):
                diffjson[kk] = vv
